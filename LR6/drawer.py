...
def draw_filled_pixels(filled_pixels, screen, color=(200, 100, 100)):
    for x, y in filled_pixels:
        rect = pg.Rect(FIELD_RECT[0] + SCALE * x, FIELD_RECT[1] + SCALE * y, SCALE, SCALE)
        pg.draw.rect(screen, color, rect)

def draw_debug_fill(debug_info, screen):
    if not debug_info:
        return
    font = pg.font.Font(None, 18)
    y = 580
    for line in debug_info[:15]:
        txt = font.render(str(line), True, (0, 0, 150))
        screen.blit(txt, (950, y))
        y += 18
